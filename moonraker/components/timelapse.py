...
class Timelapse:
    ...
    def __init__(self, confighelper: ConfigHelper) -> None:
        ...
        out_dir_cfg = confighelper.get(
            "output_path", "/userdata/timelapse/")
        temp_dir_cfg = confighelper.get(
            "frame_path", "/tmp/timelapse/")
        self.gstreamer_binary_path = confighelper.get(
            "gstreamer_binary_path", "/usr/bin/gst-launch-1.0")
        ...
        self.config: Dict[str, Any] = {
            ...
            'mode': "layermacro",
            'camera': "",
            'snapshoturl': "http://localhost/webrtc/api/frame.jpeg?src=Camera_JPEG",
        ...
        self.gstreamer_installed = os.path.isfile(self.gstreamer_binary_path)

        if not self.gstreamer_installed:
            self.config['autorender'] = False
            logging.info(f"timelapse: gstreamer not found, please install to use render functionality")
    ...
    async def start_hyperlapse(self) -> None:
        ...
        while self.printing:
            try:
                response = await self.klippy_apis.query_objects({'virtual_sdcard': None})
                vsd = response.get('virtual_sdcard', {})
                file_position = vsd.get('file_position', 0)
                if initial_position is None and file_position > 0:
                    initial_position = file_position
                elif file_position != initial_position:
                    break
            except Exception:
                logging.exception("Error querying virtual_sdcard in start_hyperlapse")
    ...
    async def render(self, webrequest=None):
        ...
        if not filelist:
            ...
            status = "running"
        elif not self.gstreamer_installed:
            msg = f"{self.gstreamer_binary_path} not found, please install gstreamer"
            status = "error"
            ...
            # GStreamer pipeline with MPP H.264 hardware encoding
            pipeline_parts = []

            # source: read sequentially named frame files
            pipeline_parts.append(f"multifilesrc location='{self.temp_dir}frame%06d.jpg' start-index=1")
            pipeline_parts.append(f"caps='image/jpeg,framerate={fps}/1'")
            ...
            if self.config['rotation'] == 90 and self.config['flip_y']:
                pipeline_parts.append("! videoflip method=upper-left-diagonal")
                ...
                pipeline_parts.append("! videoflip method=lower-left-diagonal")
            elif self.config['rotation'] == 270:
                pipeline_parts.append("! videoflip method=counter-clockwise")
            elif self.config['flip_x'] and self.config['flip_y']:
                pipeline_parts.append("! videoflip method=rotate-180")
            elif self.config['flip_x']:
                pipeline_parts.append("! videoflip method=horizontal")
            elif self.config['flip_y']:
                pipeline_parts.append("! videoflip method=vertical")

            # ensure stable framerate
            pipeline_parts.append("! videorate")

            # MPP H.264 hardware encoding
            pipeline_parts.append("! mpph264enc")
            pipeline_parts.append("! h264parse")
            pipeline_parts.append("! mp4mux")

            mp4_file = f"{self.temp_dir}{outfile}.mp4"
            pipeline_parts.append(f"! filesink location='{mp4_file}'")

            cmd = self.gstreamer_binary_path + " -e " + " ".join(pipeline_parts)
            output_ext = ".mp4"

            # log and notify ws
            logging.info(f"start GStreamer: {cmd}")
            result.update({
                'status': 'started',
            ...
            shell_cmd: SCMDComp = self.server.lookup_component('shell_command')
            self.notify_event(result)
            scmd = shell_cmd.build_shell_command(cmd, self.gstreamer_cb)
            try:
            ...
            if cmdstatus:
                status = "success"
                msg = f"Rendering Video successful: {outfile}{output_ext}"
                result.update({
                    'filename': f"{outfile}{output_ext}",
                ...
                try:
                    shutil.move(self.temp_dir + outfile + output_ext,
                                self.out_dir + outfile + output_ext)
                except OSError as err:
    ...
    def gstreamer_cb(self, response):
        # logging.debug(f"gstreamer_cb: {response}")
        self.lastcmdreponse = response.decode("utf-8")
        try:
            frame = re.search(
                r'(?<=/)(\d+)(?=/frames)', self.lastcmdreponse
            ).group()
        ...
        if self.lastrenderprogress != int(percent):
            ...
            # logging.debug(f"GStreamer Progress: {self.lastrenderprogress}% ")
            result = {
                'action': 'render',
                'status': 'running',
